  ...
  return <div className="space-y-6">
    ...
    {tab === 'wallets' && <div className="grid gap-4 lg:grid-cols-2">
      {loading ? <div className="col-span-full py-12 text-center text-sm text-gray-400">Loading wallets…</div> : wallets.length === 0 ? <div className="col-span-full rounded-xl border border-dashed border-white/15 p-10 text-center text-sm text-gray-400">No external wallet accounts exist yet. Record a project return to an external wallet, or create one from Accountant Custody.</div> : wallets.map((wallet) => <div key={wallet._id} className="rounded-2xl border border-white/10 bg-[#111827] p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2"><button onClick={() => openLedger(wallet)} className="btn-secondary inline-flex items-center gap-1.5 text-xs"><Eye size={14} />Ledger</button>{canOperate && <><button onClick={() => openReinvest(wallet)} disabled={wallet.derivedBalance <= 0} className="btn-primary inline-flex items-center gap-1.5 text-xs"><Repeat2 size={14} />Reinvest</button><button onClick={() => openLiquidate(wallet)} disabled={wallet.derivedBalance <= 0} className="btn-secondary inline-flex items-center gap-1.5 text-xs"><BanknoteArrowDown size={14} />Liquidate</button></>}</div>
      </div>)}
    </div>}
