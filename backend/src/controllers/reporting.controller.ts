...
export class ReportingController {
  ...
  static async activity(req: AuthRequest, res: Response, next: NextFunction) { try { if (!req.user) return next(createError('Authentication required', 401)); res.json({ success: true, data: await ReportingService.activity(query(req.query), req.user) }); } catch (error) { next(error); } }
